...
Every node is a `<g class="node node-<colour>">` with `data-label` (display name) and `data-tooltip` (one-sentence description) attributes. The script reads those attributes to render the tooltip.
...
- **Leave at least 20px between any two nodes.**
