...
- Skill symlinks from `~/.agents/skills/`: `ship`, `pr`, `tech-writer`, `ste`, `deck`, `roadmap`, `diagram`, `monograph`, `bootcamp`, `handoff`, `slop`, `gh-pages`, `plan-init`, `plan-add`, `plan-list`, `plan-update`, `plan-close`, `issue-init`, `issue-new`, `issue-start`, `issue-close`
...
```bash
for skill in ship pr tech-writer ste deck roadmap diagram monograph bootcamp handoff slop gh-pages plan-init plan-add plan-list plan-update plan-close issue-init issue-new issue-start issue-close; do
